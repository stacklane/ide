...
class SourceChangeSet{
    ...
    static get CHANGE_EVENT() {return "SourceChangeSet#change"};
    ...
    async _fireChangeEvent(sourceFile, changeCommand){
        window.dispatchEvent(new CustomEvent(SourceChangeSet.CHANGE_EVENT, {detail: {sessionId: this._sessionId, command: changeCommand}}));
        this._listeners.forEach((listener)=>listener(new SourceChange(changeCommand, sourceFile)));
    }

    _put(sourceFile, cmd){
        this._storage.setItem(this._key(sourceFile), JSON.stringify(cmd));
        this._fireChangeEvent(sourceFile, cmd);
    }

    _putData(sourceFile, data){
        if (sourceFile.isText) {
            this._storage.setItem(this._keyData(sourceFile), _LZString.compress(data));
        } else {
    ...
    create(newSourceFile, data){
        const createPath = newSourceFile.path;
        if (this.all().filter((change)=>change.path === createPath || change.newPath === createPath)){
            throw 'exists: ' + createPath;
        }
        const cmd = {timestamp: Date.now(), file: newSourceFile.simple, type:SourceChange.CREATE};
        this._put(newSourceFile, cmd);
        this._putData(newSourceFile, data);
    }
    ...
    delete(sourceFile){
        if (sourceFile.id){
            const cmd = {timestamp: Date.now(), file: sourceFile.simple, type: SourceChange.DELETE};
            this._put(sourceFile, cmd);
        } else {
            this._clear(sourceFile);
        }
    }
    ...
    _key(sourceFile){
        return this._keyForIdOrPath(sourceFile.handle);
    }

    _keyData(sourceFile){
        return this._prefix + '_data_' + sourceFile.handle;
    }
    ...
    stats(){
        ...
        for (let i = 0; i < this._storage.length; i++){
            ...
            switch (cmd.type){
                case SourceChange.CREATE:{
                    count.create++; break;
                }
                case SourceChange.UPDATE:
                case SourceChange.MOVE:{
                    count.update++; break;
                }
                case SourceChange.DELETE:{
                    count.delete++; break;
                }
            }
        }
    ...
    all(){
        ...
        out.sort((a,b)=>a.timestamp - b.timestamp)
            .map(value=>new SourceChange(value));
        return out;
    }
}
